...
(cd /opt/common-$CONFLUENT_VERSION && /opt/maven/bin/mvn -DskipTests install)

wget -q -O rest-utils.tar.gz $REST_UTILS_URL
tar -xzvf rest-utils.tar.gz -C /opt/
(cd /opt/rest-utils-$CONFLUENT_VERSION && /opt/maven/bin/mvn -DskipTests install)

wget -q -O connect-jdbc.tar.gz $CONNECT_JDBC_URL
tar -xzvf connect-jdbc.tar.gz -C /opt/
(cd /opt/kafka-connect-jdbc-$CONFLUENT_VERSION && /opt/maven/bin/mvn -DskipTests install)
...
(cd /opt/schema-registry-$CONFLUENT_VERSION && /opt/maven/bin/mvn -DskipTests package)
